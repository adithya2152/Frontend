...
export function ProfilePage({ onNavigate }: { onNavigate?: (page: Page, options?: { action?: string }) => void }) {
  ...
  // ✅ NEW: Reusable Stat Card component for Manager dashboard
  const StatCard = ({ icon: Icon, value, label, page }: { icon: React.ElementType, value: number, label: string, page: Page }) => (
    <div className="bg-orange-50 border border-orange-200 p-6 rounded-2xl flex flex-col justify-between shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300">
      ...
      {onNavigate && (
        <button
          ...
          className="mt-4 text-sm font-semibold text-orange-600 hover:text-orange-800 self-start"
        >
          View All &rarr;
        </button>
      )}
      ...
      <div className="animate-fadeIn">
        <div className="flex items-center space-x-4 mb-4">
          ...
          <div>
            ...
            <p className="text-gray-600 text-lg mt-1">
              {user.role === 'manager' ? 'Manager Dashboard' : 'Driver Profile'}
            </p>
          </div>
        </div>
      </div>

      {/* ✅ UPDATED: Manager Info Section */}
      {user.role === 'manager' && (
        <section className="w-full max-w-5xl mx-auto space-y-8 animate-fadeIn">
          {/* Manager's Personal Info Card */}
          ...
          {/* Manager's Statistics Cards */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <StatCard icon={Users} value={totals.drivers} label="Total Drivers" page="drivers" />
            <StatCard icon={Car} value={totals.vehicles} label="Total Vehicles" page="vehicles" />
            <StatCard icon={Map} value={totals.trips} label="Total Trips" page="trips" />
          </div>
        </section>
      )}

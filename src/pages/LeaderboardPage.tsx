  ...
  const LeaderboardPage: React.FC<LeaderboardPageProps> = ({ onNavigate }) => {
    ...
    useEffect(() => {
      if (user) {
        if (user.role === 'manager') {
          ...
          fetchDriverLeaderboard(user.id);
        }
      }
    }, [user]);
    ...
    // In your LeaderboardPage.tsx
const fetchLeaderboard = async (manager_username?: string) => {
  setLoading(true);
  try {
    // ALWAYS use the base /leaderboard route
    const url = new URL(`${API_BASE_URL}/leaderboard`);
    
    // Append manager_username as a query parameter if it exists
    if (manager_username) {
      url.searchParams.append('manager_username', manager_username);
    }
    ...
    const fetchDriverLeaderboard = async (driverId: string) => {
      ...
      try {
        ...
        if (!driverData.manager_id) {
          ...
          const data = await response.json();
          console.log("Fetched default leaderboard data:", data);
          const driversWithRank: Driver[] = data.map((driver: any, index: number) => ({
            ...driver,
            rank: index + 1
          }));
                      ...
                      <td className="px-8 py-6 whitespace-nowrap">
                        <div className="flex items-center">
                          <div className="flex-shrink-0 h-12 w-12">
                            <img
                              src={`https://api.dicebear.com/7.x/adventurer/svg?seed=${encodeURIComponent(driver.driver_id || driver.username)}`}
                              alt={driver.username}
